...
Перерывы (страница «Перерывы») работают через **свой** Supabase Realtime.
Создай проект Supabase и задай при сборке (`v2/app/.env`, см. `.env.example`):

```
VITE_SUPABASE_URL=https://<твой-проект>.supabase.co
VITE_SUPABASE_ANON_KEY=<anon/publishable ключ>
```
Если оставить пустыми — страница перерывов покажет «не настроено», остальной
сайт работает. Затем:
